...
namespace Whirlpool
{
    public class Whirlpool
    {
        ...
        private void MessageComletion()
        {
            ...
            var fileSize = (ulong)infile.Length * 8; //размер в битах
            ...
            var mybyt = BitConverter.GetBytes(fileSize);
            for (var i = 0; i < 8; i++)
        ...
        private void Description()
        {
            ...
            var m = new byte[8, 8]; //Блок сообщения
            long position = 0;
        ...
        private string Output(byte[,] arr)
        {
            var buf = new byte[64];
            for (var i = 0; i < 8; i++)
                for (var j = 0; j < 8; j++)
